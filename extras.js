(() => {
  ...
  hub.innerHTML=`<div class="study-hub-title"><div><p class="eyebrow">CENTRAL DE ESTUDOS</p><h2>Minhas ferramentas</h2></div><button id="toggleStudyHub" class="secondary-button" aria-expanded="false">Abrir ferramentas</button></div>
    <div id="studyHubBody" class="study-hub-body hidden">
      ...
      <article class="study-tool"><h3>🛡 Segurança</h3><p>Proteja o caderno com um PIN de 4 a 8 números.</p><div class="pin-input-row"><input id="notebookPinInput" type="password" inputmode="numeric" maxlength="8" pattern="[0-9]{4,8}" placeholder="Digite seu PIN" aria-label="Novo PIN"><button id="togglePinVisibility" type="button" aria-label="Mostrar PIN" aria-pressed="false" title="Mostrar PIN">👁</button></div><div class="tool-row"><button id="manageNotebookPin">Salvar PIN</button><button id="removeNotebookPin" class="secondary-button">Remover PIN</button></div><small id="pinSettingsStatus" role="status" aria-live="polite"></small></article>
    </div>`;
  ...
  togglePinVisibility.onclick=()=>{const showing=notebookPinInput.type==='text';notebookPinInput.type=showing?'password':'text';togglePinVisibility.textContent=showing?'👁':'🙈';togglePinVisibility.setAttribute('aria-label',showing?'Mostrar PIN':'Esconder PIN');togglePinVisibility.setAttribute('title',showing?'Mostrar PIN':'Esconder PIN');togglePinVisibility.setAttribute('aria-pressed',String(!showing));notebookPinInput.focus();};
